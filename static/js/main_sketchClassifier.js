...
function addStreetPopUpSM(layer){
	...
	if (options.length != 0){
        ...
        for (var i = 0; i <options.length; i++) {
			...
			var x = document.createTextNode(options[i]);
			opt.appendChild(x);
			...
			// add names for all objects loaded from geoJson
			LayerName = layer.feature.properties.id;
			if (LayerName == options[i]){
		...
		// handle checkbox for the "RouteSegment" statetment
        isRoute.addEventListener("click",RouteClickedFunction,false);
        function RouteClickedFunction(){
			if (isRoute.checked == true){
				console.log("True");
				isRoute.setAttribute("value","Yes");
				layer.feature.properties.isRoute = isRoute.value;
				console.log(isRoute.value);
			}else if (isRoute.checked == false ){
				console.log("False");
				isRoute.setAttribute("value", null);
				layer.feature.properties.isRoute = isRoute.value;
...
function addLandmarkPopupSM(layer){
    ...
    if (options.length != 0){
        ...
        function clickedFunction3(){
            if(isLandmark.checked == true){
				isLandmark.setAttribute("value","Landmark");
				console.log(isLandmark.value);
				layer.feature.properties.feat_type = isLandmark.value;
				isCityblock.checked = false;
			} else if(isLandmark.checked == false){
				isCityblock.checked = true;
				isCityblock.setAttribute("value","Cityblock");
				console.log(isCityblock.value);
				layer.feature.properties.feat_type = isCityblock.value;
			}
        };
